#!/usr/bin/python
from mininet.topo import Topo
from mininet.net import Mininet
from mininet.node import OVSKernelSwitch, RemoteController
from mininet.cli import CLI
from mininet.link import TCLink

class /home/vagrant/comnetsemu/app/new_topologies/InternetZooTopologies/DatasetGML/Bandcon(Topo):
 def __init__(self):
  Topo.__init__(self)
  # Adding Switches
  s0 = self.addSwitch('00', dpid='0000000000000002')
  s1 = self.addSwitch('11', dpid='0000000000000003')
  s2 = self.addSwitch('22', dpid='0000000000000004')
  s3 = self.addSwitch('33', dpid='0000000000000005')
  s4 = self.addSwitch('44', dpid='0000000000000006')
  s5 = self.addSwitch('55', dpid='0000000000000007')
  s6 = self.addSwitch('66', dpid='0000000000000008')
  s7 = self.addSwitch('77', dpid='0000000000000009')
  s8 = self.addSwitch('88', dpid='000000000000000a')
  s9 = self.addSwitch('99', dpid='000000000000000b')
  s10 = self.addSwitch('1010', dpid='000000000000000c')
  s11 = self.addSwitch('1111', dpid='000000000000000d')
  s12 = self.addSwitch('1212', dpid='000000000000000e')
  s13 = self.addSwitch('1313', dpid='000000000000000f')
  s14 = self.addSwitch('1414', dpid='0000000000000010')
  s15 = self.addSwitch('1515', dpid='0000000000000011')
  s16 = self.addSwitch('1616', dpid='0000000000000012')
  s17 = self.addSwitch('1717', dpid='0000000000000013')
  s18 = self.addSwitch('1818', dpid='0000000000000014')
  s19 = self.addSwitch('1919', dpid='0000000000000015')
  s20 = self.addSwitch('2020', dpid='0000000000000016')
  s21 = self.addSwitch('2121', dpid='0000000000000017')

  # Adding Links
  link1_conf = {'delay': '20ms', 'use_tbf': True, 'bw': 20, 'max_queue_size': 10, 'burst': 1000000}
  link2_conf = {'delay': '25ms', 'use_tbf': True, 'bw': 50, 'max_queue_size': 10, 'burst': 1000000}
  self.addLink(s0, s3, **link1_conf)
  self.addLink(s0, s12, **link1_conf)
  self.addLink(s1, s19, **link1_conf)
  self.addLink(s1, s6, **link2_conf)
  self.addLink(s2, s3, **link1_conf)
  self.addLink(s2, s13, **link1_conf)
  self.addLink(s4, s8, **link2_conf)
  self.addLink(s4, s7, **link2_conf)
  self.addLink(s5, s8, **link2_conf)
  self.addLink(s5, s14, **link2_conf)
  self.addLink(s6, s7, **link1_conf)
  self.addLink(s7, s9, **link2_conf)
  self.addLink(s7, s19, **link2_conf)
  self.addLink(s7, s17, **link2_conf)
  self.addLink(s8, s14, **link2_conf)
  self.addLink(s10, s18, **link2_conf)
  self.addLink(s10, s19, **link1_conf)
  self.addLink(s11, s18, **link1_conf)
  self.addLink(s11, s21, **link2_conf)
  self.addLink(s12, s13, **link1_conf)
  self.addLink(s12, s21, **link2_conf)
  self.addLink(s13, s19, **link1_conf)
  self.addLink(s14, s17, **link1_conf)
  self.addLink(s14, s15, **link2_conf)
  self.addLink(s15, s16, **link2_conf)
  self.addLink(s16, s17, **link1_conf)
  self.addLink(s17, s18, **link2_conf)
  self.addLink(s17, s19, **link1_conf)

topos = { '/home/vagrant/comnetsemu/app/new_topologies/InternetZooTopologies/DatasetGML/Bandcon': (lambda: /home/vagrant/comnetsemu/app/new_topologies/InternetZooTopologies/DatasetGML/Bandcon()) }