#!/usr/bin/python
from mininet.topo import Topo
from mininet.net import Mininet
from mininet.node import OVSKernelSwitch, RemoteController
from mininet.cli import CLI
from mininet.link import TCLink


class Packetexchange(Topo):
    def __init__(self):
        Topo.__init__(self)
        # Adding Switches
        s0 = self.addSwitch("miami0", dpid="0000000000000002")
        s1 = self.addSwitch("london1", dpid="0000000000000003")
        s2 = self.addSwitch("ashburn2", dpid="0000000000000004")
        s3 = self.addSwitch("newyork33", dpid="0000000000000005")
        s4 = self.addSwitch("paris4", dpid="0000000000000006")
        s5 = self.addSwitch("milan5", dpid="0000000000000007")
        s6 = self.addSwitch("amsterda6", dpid="0000000000000008")
        s7 = self.addSwitch("frankfur7", dpid="0000000000000009")
        s8 = self.addSwitch("hongkong8", dpid="000000000000000a")
        s9 = self.addSwitch("singapor9", dpid="000000000000000b")
        s10 = self.addSwitch("losangel0", dpid="000000000000000c")
        s11 = self.addSwitch("kansasci1", dpid="000000000000000d")
        s12 = self.addSwitch("chicago12", dpid="000000000000000e")
        s13 = self.addSwitch("seattle13", dpid="000000000000000f")
        s14 = self.addSwitch("sanfranc4", dpid="0000000000000010")
        s15 = self.addSwitch("sanjose15", dpid="0000000000000011")
        s16 = self.addSwitch("houston16", dpid="0000000000000012")
        s17 = self.addSwitch("lasvegas7", dpid="0000000000000013")
        s18 = self.addSwitch("phoenix18", dpid="0000000000000014")
        s19 = self.addSwitch("dallas19", dpid="0000000000000015")
        s20 = self.addSwitch("atlanta20", dpid="0000000000000016")

        # Adding Links
        self.addLink(s0, s16)
        self.addLink(s0, s20)
        self.addLink(s1, s2)
        self.addLink(s1, s3)
        self.addLink(s1, s4)
        self.addLink(s1, s6)
        self.addLink(s2, s12)
        self.addLink(s2, s20)
        self.addLink(s3, s4)
        self.addLink(s4, s7)
        self.addLink(s5, s7)
        self.addLink(s6, s7)
        self.addLink(s8, s9)
        self.addLink(s8, s14)
        self.addLink(s9, s10)
        self.addLink(s10, s18)
        self.addLink(s10, s15)
        self.addLink(s10, s17)
        self.addLink(s11, s19)
        self.addLink(s11, s12)
        self.addLink(s11, s13)
        self.addLink(s11, s14)
        self.addLink(s13, s14)
        self.addLink(s14, s15)
        self.addLink(s16, s19)
        self.addLink(s18, s19)
        self.addLink(s19, s20)


topos = {"Packetexchange": (lambda: Packetexchange())}
